...
| Unconstrained generation prompt — question 5 | Unconstrained | Available (question 5 only) | `prompts/unconstrained_generation_prompt_question5.pdf` (+ `.txt`) | Original (author's prompt) | Yes — the Q5 generation prompt | PDF is authoritative; the text copy is an auto-extraction. Contains no embedded FCI text/image. |
| Unconstrained generation prompts — questions 16, 17, 22, 23 | Unconstrained | **Not retained** | — | — | Were used during the study | Not provided in the available research records; not reconstructed or invented. |
...
- **Not retained (do not exist in the available records):** the constrained
  extraction JSON configurations; the exact unconstrained generation prompts for
  questions 16, 17, 22, and 23; a record identifying which of the 36 constrained
  runs were the five scored outputs.
