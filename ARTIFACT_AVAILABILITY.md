...
| NewtonSimAI source code | Constrained | Available | `NewtonSimAI_source/` | Original source code | Indirectly — the implementation that produced the constrained outputs, and the shared backend the constrained sims use | The tool itself. `node_modules/` and `.env` excluded. |
| Five constrained scored generations | Constrained | Available | `constrained_simulations/Q05..Q23/` | Original generated artifacts (NewtonSimAI) | Yes — the scored constrained artifacts | One generation per question, identified by the study author who performed the scoring. Each is the tool's emitted `index.html`, `projectile.js`, `styles_projectiles.css`; served by the shared NewtonSimAI backend. |
...
- **Not retained (do not exist in the available records):** the constrained
  extraction JSON configurations (the config was injected into the generated
  `projectile.js`, not saved separately). (The five scored constrained
  generations **are** archived, and the unconstrained generation prompt **is**
  available — a single prompt used for all five questions.)
